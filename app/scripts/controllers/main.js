...
angular.module('prototypoApp')
	.controller('MainCtrl', function( $scope, $routeParams, $parse, $q, $location, History, RemoveDiacritics, Typeface, FontValues, AppValues, Font, Glyph ) {
		var calculated = [];
		...
		Typeface.get( /* $routeParams.typeface */ 'default' )
			/*
			...
			.then(function( typeface ) {
				...
				$scope.resetFontValues = function() {
					...
					// set font name in fontValues
					$scope.fontValues[ '_name' ] = $routeParams.typeface;
					updateCalculatedParams( $scope.fontValues );
				};

				$scope.fontName = function() {

					// close input panel
					$scope.appValues.displayNewFontInput = !$scope.appValues.displayNewFontInput;

					// We have to format the name to fit in url format
					// Need to store smart name in object 

					$scope.fontName = $('input[name="inputFontName"]').val();
					if ($scope.fontName != null) {
						/* change url to allow hoodie to store the new font with a smart name encoded */ 
				...
				promises.push( AppValues.get({ typeface: /* $routeParams.typeface */ 'default' })
					.then(
						function done( data ) {
	...
	.factory('RemoveDiacritics', function () {
		// Hoodie ID only accepts a-Z 0-9 chars
		// TODO: better solution to make fontName url acceptable by hoodie			
